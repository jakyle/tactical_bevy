...
use crate::GameState;
use crate::{actions::Actions, map::MapTransform};
use bevy::prelude::*;
...
struct PlacePlayerOnTileEvent{ pub map_id: u16, pub layer_id: u16 }
...
#[derive(Debug)]
pub struct IsMoving(pub Option<Vec2>);
...
fn place_sprite_on_tile(
    ...
    for event in place_player_reader.iter() {
        for (grid_position, mut sprite, mut transform) in player_query.iter_mut() {
            if let Some((_, layer)) = map_query.get_layer(event.map_id, event.layer_id) {

                let tile_size = layer.settings.tile_size;
                let border_offset = Vec2::new(5., 5.);
                sprite.size = tile_size - border_offset;

                transform.translation = grid_position.tile_center_translation(tile_size, 1.);
            }
        }
    }
}
...
fn start_move(
    time: Res<Time>,
    actions: Res<Actions>,
    mut player_query: Query<&mut IsMoving, With<Player>,>,
) {
    if actions.player_movement.is_none() {
        return;
    }

    for mut is_moving in player_query.iter_mut() {
        ...
        **is_moving = actions.player_movement;
    }
}
...
fn move_player(
    ...
    for (mut is_moving, mut transform, mut grid_position) in player_query.iter_mut() {
        if let Some(direction) = **is_moving {

            println!("{:?}", grid_position);
            println!("before neighbors");
            ...
            if let Some((neighbor, Some(_))) = is_neighbor {
                ...
                let (_, layer) = map_query.get_layer(0u16, 0u16).unwrap(); // TODO handle option
                let tile_size = layer.settings.tile_size;
                let destination = Vec3::new(
                    (neighbor.x as f32 * tile_size.x) + (tile_size.x / 2.),
                    (neighbor.y as f32 * tile_size.y) + (tile_size.y / 2.), 
                1.
                );
                ...
                if current_position.distance(destination) > 0.5 {
                    ...
                    **is_moving = None;
                    grid_position.0 = UVec2::new(neighbor.x as u32, neighbor.y as u32);
                }
            } else {
                **is_moving = None;
            }
